...
function social_links()
{
    ...
    foreach ($social_links as $link => $key) {
        if (!empty($key)) {
            switch ($link) {
                case 'facebook':
                    echo '<a target="_blank" href="https://facebook.com/' . $key . '" rel="nofollow" class="btn btn-icon-only btn-facebook rounded-circle"><span class="btn-inner--icon"><i class="fab fa-facebook"></i></span></a>';
                    break;
                case 'twitter':
                    echo '<a target="_blank" href="https://twitter.com/' . $key . '" rel="nofollow" class="btn btn-icon-only btn-twitter rounded-circle"><span class="btn-inner--icon"><i class="fab fa-twitter"></i></span></a>';
                    break;
                case 'youtube':
                    echo '<a target="_blank" href="https://youtube.com/' . $key . '" rel="nofollow" class="btn btn-icon-only btn-youtube rounded-circle"><span class="btn-inner--icon"><i class="fab fa-youtube"></i></span></a>';
                    break;
                case 'instagram':
                    echo '<a target="_blank" href="https://instagram.com/' . $key . '" rel="nofollow" class="btn btn-icon-only btn-instagram rounded-circle"><span class="btn-inner--icon"><i class="fab fa-instagram"></i></span></a>';
                    break;
            }
        }
    }
    ...
    foreach (glob(__DIR__ . "/../../language/*.php") as $filename) {
        if (basename($filename) != "index.php") {
            ...
            if (language_exists($language) === true) {
                ...
                $language_info["icon"] = '<img src="' . $website_url . '/template/sober/img/flags/' . $language . '.svg" class="img-fluid w-25" alt="' . $language . ' flag"';
                $flag = sprintf('<span class="mr-1 mt-1">%s</span>', $language_info["icon"]);
                printf('<a class="dropdown-item" href="?lang=%s">%s %s</a>', $language, $flag, $language_info["name"]);
            }
        }
    }
}
