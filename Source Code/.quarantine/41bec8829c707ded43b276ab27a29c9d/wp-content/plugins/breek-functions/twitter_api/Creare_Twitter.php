...
class Creare_Twitter
{
	...
	private function getTimeAgo($time)
	{
  			...
  			/********************* Checking the times and returning correct value */
		   	if($theminutes < 60){
				if($theminutes < 1){
					$timemessage =  esc_html__("Less than 1 minute ago", 'breek');
				} else if($theminutes == 1) {
				 	$timemessage = sprintf( esc_html__("%s minute ago", 'breek'), $theminutes);
				} else {
				 	$timemessage = sprintf( esc_html__("%s minutes ago", 'breek'), $theminutes);
				}
			} else if($theminutes > 60 && $thedays < 1){
				 if($thehours == 1){
				 	$timemessage = sprintf( esc_html__("%s hour ago", 'breek'), $thehours);
				 } else {
				 	$timemessage = sprintf( esc_html__("%s hours ago", 'breek'), $thehours);
				 }
			} else {
				 if($thedays == 1){
				 	$timemessage = sprintf( esc_html__("%s day ago", 'breek'), $thedays);
				 } else {
				 	$timemessage = sprintf( esc_html__("%s days ago", 'breek'), $thedays);
				 }
			}
		return $timemessage;	
    }
