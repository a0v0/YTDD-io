...
class Buy_Me_A_Coffee_Admin
{
    ...
    public function name_post()
    {
        ...
        $response = wp_remote_post('https://buymeacoffee.com/api/check_availability', array(
            'body'    => array('project_slug' => $name),
        ));
        $decodejs = json_decode($response['body']);


        if ($decodejs->status == "FAIL" && $decodejs->errors->project_slug == "The username " . $name . " is already taken.") {
    ...
    public function widget_post()
    {
        ...
        if ($_POST['save']  && $_POST['toogle_switch_widget'] == null) {
            $data = array(
                ...
                'side_spacing' => $_POST['side_spacing'],
                'bottom_spacing' => $_POST['bottom_spacing'],
                'widget_isactive' => '1'
            );
        }
